...
def generate_adversarial_patch(attack, image, step_num):
    ...
    image_copies = np.vstack([image.image_as_np_array]*2)
    ...
    for step in range(previous_num_steps, previous_num_steps + step_num):
        ...
        attack.generate(x=image_copies, print_nth_num=500, y=None)                    
...
def generate_rrap_for_image(image_name):
    ...
    attack = RobustDPatch(estimator=FRCNN, max_iter=1000, batch_size=1, verbose=False, rotation_weights=(1,0,0,0), 
                        ...
                        image_to_patch = image, training_data_path = training_data_path, perceptibility_learning_rate = 5.0, 
                        detection_learning_rate = 5.0, training_data = get_previous_training_data(training_data_path))

    generate_adversarial_patch(attack, image, step_num = 15)
